...
from problems import make_problem_3d
from solvers.gs_3d import solve as solve_gs
from solvers.jacobi_3d import solve as solve_jacobi
from solvers.mg_3d import solve as solve_mg
from solvers.sor_3d import solve as solve_sor
...
WARMUP_MAX_ITER = 10
TOL = 1e-9
MAX_SINGLE_RUN_MS = 10_000.0
MG_OMEGA = 1.25
MG_NU = 3
MG_COARSE_STEPS = 16
MG_MAX_ITER = 150

# Calibrated on the conda "mg" environment so each timed solve stays under
# 10 seconds on this workstation. Keep these conservative for repeatability.
SOLVER_SIZES = (15, 31, 47, 63)
RB_SOR_SIZES = (31, 63, 95, 127, 159)
MG_SIZES = (15, 31, 63, 127, 255, 383)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the calibrated Python 3D benchmark suite. The default grid "
            "sizes are chosen so each timed solve stays under 10 seconds."
        )
    )
    parser.add_argument(
        "--suite",
    ...
    parser.add_argument(
        ...
        help="Number of warmup runs before timing each row.",
    )
    parser.add_argument(
        "--timed-runs",
        type=int,
        default=DEFAULT_TIMED_RUNS,
        help="Number of timed runs per benchmark row.",
    )
    parser.add_argument(
        "--max-single-run-ms",
        type=float,
        default=MAX_SINGLE_RUN_MS,
        help="Warn if any timed solve exceeds this duration.",
    )
    ...
    if args.max_single_run_ms <= 0.0:
        parser.error("max-single-run-ms must be positive")
    return args
...
def time_solver(
    ...
    return last_result, TimingStats(
        mean_ms=mean_ms,
        std_ms=std_ms,
        max_ms=max(samples),
    )
...
def make_row(
    ...
    problem = make_problem_3d(CASE_NAME, grid_size, dtype=np.float64)
    ...
    metrics_problem = make_problem_3d(CASE_NAME, grid_size, dtype=np.float64)
    error_l2, error_linf = error_metrics_3d(metrics_problem, result.phi)
...
def make_solver_comparison_rows(warmup_runs: int, timed_runs: int) -> list[BenchmarkRow]:
    rows: list[BenchmarkRow] = []
...
def make_mg_compare_rows(warmup_runs: int, timed_runs: int) -> list[BenchmarkRow]:
    rows: list[BenchmarkRow] = []
...
def run_suite(suite: str, warmup_runs: int, timed_runs: int) -> list[BenchmarkRow]:
    rows: list[BenchmarkRow] = []

    if suite in {SUITE_SOLVER_COMPARISON, "all"}:
        rows.extend(make_solver_comparison_rows(warmup_runs, timed_runs))

    if suite in {SUITE_MG_COMPARE, "all"}:
        rows.extend(make_mg_compare_rows(warmup_runs, timed_runs))
...
def write_simple_csv_header(writer: csv.writer) -> None:
    writer.writerow(
        ["solver", "grid_size", "iterations", "mean_time_ms", "std_time_ms", "max_time_ms"]
    )
...
def warn_if_over_limit(rows: list[BenchmarkRow], max_single_run_ms: float) -> None:
    over_limit = [row for row in rows if row.max_time_ms > max_single_run_ms]
    for row in over_limit:
        print(
            "WARNING: "
            f"{row.solver} n={row.grid_size} max_time_ms={row.max_time_ms:.3f} "
            f"exceeded limit {max_single_run_ms:.3f}",
            file=sys.stderr,
        )
...
def main() -> int:
    args = parse_args()
    print(
        f"Running 3D benchmark suite '{args.suite}' on backend '{BACKEND_NAME}'",
        file=sys.stderr,
    )
    rows = run_suite(args.suite, args.warmup_runs, args.timed_runs)
    warn_if_over_limit(rows, args.max_single_run_ms)
