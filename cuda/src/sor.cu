...
template <typename Real>
void validate_sor_inputs(const Problem3D<Real>& problem, const SolveOptions<Real>& options) {
    if (options.tol <= Real{}) {
        throw std::invalid_argument("tol must be positive");
    }
    if (options.max_iter < 1) {
...
SolveResult3D solve_sor(const Problem3D<Real>& problem, const SolveOptions<Real>& options) {
    const cuda::detail::ScopedNvtxRange solve_range{"cuda::solve_sor_3d"};
    validate_sor_inputs(problem, options);
    cuda::ensure_device_available();
    ...
    for (std::size_t iteration = 1; iteration <= options.max_iter; ++iteration) {
        const cuda::detail::ScopedNvtxRange iteration_range{"cuda::sor_iteration_3d"};
        cuda::run_rb_sor_steps(phi, rhs, problem.h, omega, 1);
