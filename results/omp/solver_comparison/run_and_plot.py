...
def main():
    grid_sizes = np.array([16, 24, 32, 48, 64, 96, 128], dtype=int) - 1
    dtype_cli = "double"
    tol = 1e-10
    max_iter = 50000
    ...
    for case in CASES:
        ...
        plt.tight_layout()
        out_path = Path(__file__).resolve().parent / f"plots_{case}.png"
        plt.savefig(out_path, dpi=300)
        plt.close(fig)
        print(f"Plot saved to {out_path}")

        csv_path = Path(__file__).resolve().parent / f"results_{case}.csv"
