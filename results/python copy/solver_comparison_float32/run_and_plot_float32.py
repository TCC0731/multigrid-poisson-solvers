...
SOLVER_MODULES = {
    ...
    "MG V-cycle": ("solvers.mg_2d", "v"),
    "MG W-cycle": ("solvers.mg_2d", "w"),
}
...
def main():
    grid_sizes = np.array([16,24,32,48,64,96,128])-1
    dtype = np.float32
    tol = 1e-2
    max_iter = 100000
    ...
    for case in CASES:
        ...
        h_fit = 1.0 / (n_fit + 1)
        err_fit = results["Jacobi"]['error_l2'][0] * (h_fit / h_fit[0]) ** 2
        ...
        h_fit = 1.0 / (n_fit + 1)
        err_fit_inf = results["Jacobi"]['error_linf'][0] * (h_fit / h_fit[0]) ** 2
        ...
        plt.tight_layout()
        out_path = os.path.join(os.path.dirname(__file__), f'plots_{case}.png')
        plt.savefig(out_path, dpi=300)
        plt.close(fig)
        print(f"Plot saved to {out_path}")

        import csv
        csv_path = os.path.join(os.path.dirname(__file__), f'results_{case}.csv')
