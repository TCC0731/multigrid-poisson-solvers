...
def resolve_executable() -> Path:
    override = os.environ.get("POISSON_OMP_BIN")
    if override:
    ...
    *,
    ...
    if not exe.is_file():
        raise FileNotFoundError(
            f"OpenMP executable not found at {exe}. "
            "Build the project first or set POISSON_OMP_BIN."
        )
    ...
    if len(rows) != 1:
        raise RuntimeError(
            "Expected exactly one CSV row from the OpenMP executable.\n"
            f"Command: {' '.join(cmd)}\n"
            f"stdout:\n{completed.stdout}\n"
            f"stderr:\n{completed.stderr}"
        )
