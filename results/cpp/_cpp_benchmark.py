...
DEFAULT_EXECUTABLE = REPO_ROOT / "build" / "poisson_cpp"
...
def resolve_executable() -> Path:
    override = os.environ.get("POISSON_CPP_BIN")
    if override:
    ...
    *,
    ...
    if not exe.is_file():
        raise FileNotFoundError(
            f"C++ executable not found at {exe}. "
            "Build the project first or set POISSON_CPP_BIN."
        )
    ...
    if len(rows) != 1:
        raise RuntimeError(
            "Expected exactly one CSV row from the C++ executable.\n"
            f"Command: {' '.join(cmd)}\n"
            f"stdout:\n{completed.stdout}\n"
            f"stderr:\n{completed.stderr}"
        )
