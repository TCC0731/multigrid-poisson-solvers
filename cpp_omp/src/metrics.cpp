...
#ifdef _OPENMP
#pragma omp parallel for collapse(2) if(omp_config::should_parallel_2d(r.size(), r.size())) reduction(+:sum_sq) schedule(static)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for collapse(2) if(omp_config::should_parallel_3d(r.size(), r.size(), r.size())) reduction(+:sum_sq) schedule(static)
#endif
    for (std::size_t i = 0; i < r.size(); ++i) {
        for (std::size_t j = 0; j < r.size(); ++j) {
#ifdef _OPENMP
...
#ifdef _OPENMP
#pragma omp parallel for if(omp_config::should_parallel_2d(interior_end - 1, interior_end - 1)) reduction(+:res_total, rhs_total) schedule(static)
#endif
    for (std::size_t i = 1; i < interior_end; ++i) {
#ifdef _OPENMP
#pragma omp simd reduction(+:res_total, rhs_total)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for collapse(2) if(omp_config::should_parallel_3d(interior_end - 1, interior_end - 1, interior_end - 1)) reduction(+:res_total, rhs_total) schedule(static)
#endif
    for (std::size_t i = 1; i < interior_end; ++i) {
        for (std::size_t j = 1; j < interior_end; ++j) {
#ifdef _OPENMP
...
#ifdef _OPENMP
#pragma omp parallel for if(omp_config::should_parallel_2d(interior_end - 1, interior_end - 1)) reduction(+:sum_sq) reduction(max:max_abs) schedule(static)
#endif
    for (std::size_t i = 1; i < interior_end; ++i) {
#ifdef _OPENMP
#pragma omp simd reduction(+:sum_sq) reduction(max:max_abs)
#endif
        for (std::size_t j = 1; j < interior_end; ++j) {
...
#ifdef _OPENMP
#pragma omp parallel for collapse(2) if(omp_config::should_parallel_3d(interior_end - 1, interior_end - 1, interior_end - 1)) reduction(+:sum_sq) reduction(max:max_abs) schedule(static)
#endif
    for (std::size_t i = 1; i < interior_end; ++i) {
        for (std::size_t j = 1; j < interior_end; ++j) {
#ifdef _OPENMP
