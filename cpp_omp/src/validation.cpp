...
bool finite_grid(const Grid3D<Real>& grid) {
    bool all_finite = true;
#ifdef _OPENMP
#pragma omp parallel for collapse(3) if(omp_config::should_parallel_3d(grid.size(), grid.size(), grid.size())) reduction(&&:all_finite) schedule(static)
#endif
    for (std::size_t i = 0; i < grid.size(); ++i) {
        for (std::size_t j = 0; j < grid.size(); ++j) {
            for (std::size_t k = 0; k < grid.size(); ++k) {
                all_finite = all_finite && std::isfinite(grid.unchecked(i, j, k));
            }
        }
    }
    return all_finite;
...
#ifdef _OPENMP
#pragma omp parallel for collapse(2) if(omp_config::should_parallel_2d(array_n, array_n)) reduction(max:boundary_error) schedule(static)
#endif
    for (std::size_t i = 0; i < array_n; ++i) {
        for (std::size_t j = 0; j < array_n; ++j) {
            boundary_error = std::max(
...
#pragma omp parallel for collapse(2) if(omp_config::should_parallel_2d(array_n, array_n)) reduction(max:boundary_error) schedule(static)
    ...
    double interior_max_abs = 0.0;
#ifdef _OPENMP
#pragma omp parallel for collapse(2) if(omp_config::should_parallel_3d(interior_end - 1, interior_end - 1, interior_end - 1)) reduction(max:interior_max_abs) schedule(static)
#endif
    for (std::size_t i = 1; i < interior_end; ++i) {
        for (std::size_t j = 1; j < interior_end; ++j) {
#ifdef _OPENMP
