...
#ifdef _OPENMP
#pragma omp parallel for collapse(2) if(omp_config::should_parallel_3d(array_n, array_n, array_n)) schedule(static)
#endif
    for (std::size_t i = 0; i < array_n; ++i) {
        for (std::size_t j = 0; j < array_n; ++j) {
            const Real x = static_cast<Real>(i) * h;
...
#ifdef _OPENMP
#pragma omp parallel for collapse(2) if(omp_config::should_parallel_2d(array_n, array_n)) schedule(static)
#endif
    for (std::size_t i = 0; i < array_n; ++i) {
        for (std::size_t j = 0; j < array_n; ++j) {
