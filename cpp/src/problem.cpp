...
Problem3D<Real> make_problem_3d_impl(Case3D case_id, std::size_t interior_n) {
    ...
    for (std::size_t i = 0; i < array_n; ++i) {
        ...
        for (std::size_t j = 0; j < array_n; ++j) {
            ...
            for (std::size_t k = 0; k < array_n; ++k) {
                ...
                problem.exact(i, j, k) = functions.exact(x, y, z);
                problem.rhs(i, j, k) = functions.rhs(x, y, z);
            }
        }
    }
    ...
    for (std::size_t i = 0; i < array_n; ++i) {
        for (std::size_t j = 0; j < array_n; ++j) {
            problem.phi0(i, j, 0) = problem.exact(i, j, 0);
            problem.phi0(i, j, array_n - 1) = problem.exact(i, j, array_n - 1);
        }
    }

    for (std::size_t i = 0; i < array_n; ++i) {
        for (std::size_t k = 0; k < array_n; ++k) {
            problem.phi0(i, 0, k) = problem.exact(i, 0, k);
            problem.phi0(i, array_n - 1, k) = problem.exact(i, array_n - 1, k);
        }
    }

    for (std::size_t j = 0; j < array_n; ++j) {
        for (std::size_t k = 0; k < array_n; ++k) {
            problem.phi0(0, j, k) = problem.exact(0, j, k);
            problem.phi0(array_n - 1, j, k) = problem.exact(array_n - 1, j, k);
        }
    }
